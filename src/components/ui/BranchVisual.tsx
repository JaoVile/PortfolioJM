...
interface BranchVisualProps {
  theme: "light" | "dark";
  onToggle?: (isActive: boolean) => void;
}

export const BranchVisual: React.FC<BranchVisualProps> = ({ theme, onToggle }) => {
  ...
  const ClickableLine = (
    <motion.div
      ...
      className={`absolute w-full h-[1px] top-1/2 left-0 -translate-y-1/2 origin-left transition-colors duration-700`}
      style={{
  ...
  return (
    <motion.div
      className="hidden md:block w-1/3 h-20 mb-4 relative cursor-pointer"
      initial={{ scaleX: 0 }}
